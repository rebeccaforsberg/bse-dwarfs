...
c Input
      ...
      real*8  getmass,dm91,r10,getzcum,semi,opik
...
*   
      ...
      real*8 gammaqs, gammaql,a1,qb,Xb
...
c      open(22,file="population.in",status="old")
...
c Counters of ignored and observed systems
      ...
      do while (unfinished)
         istar = nobs + noff
c Check whether binary / single star is detected
         seen = .false.
...
c Power-law mass ratio
            ...
            m2 = 0.d0
c Slopes in the power law comes from Duchene, Kraus, (2013) and Moe, Di Stefano (2017)           
...
c Galactic model -- include ability to reject Galactic positions without doing binary integration
         ...
         do while (galReject)
            ...
            if (Dgal.gt.8.d0) then ! Reject stars more than 0.3kpc from Sun
c Selection criteria to mimic APOGEE
c            if (lgal.lt.24.d0 .or. lgal.gt.24.d1 .or. abs(bgal).gt.16.d0) then
               galReject = .true.
            else
               galReject = .false.
            end if
...
c         if (m1.le.0.5d0 .and. Dgal.ge.0.1d0) then 
c            nignored = nignored + 1
c            go to 200
call         end if
...
c Metallicity
            ...
            if (.not.single) call countEvents(eventcounts, rlofEventString)
...
c     &                     (eventcounts(n),n=1,3), rlofeventstring
                  call flush(96)
                  call flush(26)
               else
...
c Post-binary primary
     ...
     &                      magsun1,(magsun1-mags1(n),n=1,nbands),
     &                      status1,(mags1(n),n=1,nbands),Mj,Mk,mbolcomb,mH,
     &                      (eventcounts(n),n=1,3), rlofeventstring   
               else
                  nunseenb = nunseenb + 1
               end if
...
c Post-binary secondary
     ...
     &                      logg2,
     &                      magsun2,
     &                      (magsun2-mags2(n),n=1,nbands),status2, 
     &                      (mags2(n),n=1,nbands),Mj,Mk,mbolcomb,mH,
     &                      (eventcounts(n),n=1,3), rlofeventstring               
               else
                  nunseenb = nunseenb + 1
               end if
...
c Disrupted binary
     ...
     &                      (magsun2-mags2(n),n=1,nbands),status1,status2, 
     &                      (mags1(n),n=1,nbands),(mags2(n),n=1,nbands),Mj,Mk,mbolcomb,mH,
     &                      (eventcounts(n),n=1,3), rlofeventstring
c                      go to 70
                   else
                      nunseenb = nunseenb + 1
...
c Simple test for observation of binary
     ...
     :                           xdiffs(1),istar,ota,otb,ot2,Mj,Mk,mbolcomb,mH,fratio,kw,kw2)
c Unresolved binary
                   if (ot2) then
                      iwhich = 0
     ...
     &                      meanv,sdv,(xdiffs(n),n=1,28),(vdiffs(n),n=1,28),iwhich,
     &                      (mags1(n),n=1,nbands),(mags2(n),n=1,nbands),Mj,Mk,mbolcomb,mH,fratio,
     &                      (eventcounts(n),n=1,3), rlofeventstring
c Resolved or unseen binary
                   else
                      iwhich = 0
                      if (ota) iwhich = iwhich + 1
                      if (otb) iwhich = iwhich + 2
...
c If we have seen a binary then run analysis to check for RLOF output
         if (seen .and..not. single) then
            if (eventcounts(1).ne.0) then
               write (*,*) 'ISTAR=', istar
               call printbpp
c            else
c               write (*,*) 'ISTAR=', istar, ' is uninteresting'
            end if
            if (eventcounts(1).lt.0) then
               write (*,*) 'ERROR!!!!'
               stop
            end if
         end if
...
c If we skipped because of  a tedious star just carry out round the loop
 ...
 81   FORMAT('#Star #TIME  kw  kw2  mass  massc',
     &       '   logr  logl',
 ...
 9    FORMAT(i9,1x,f10.4,2i3,10(1x,f10.4),7(1x,e15.4),10f15.4,2i6,
     :        10(1x,f10.4),3(1x,i1),1x,a30)
 99   FORMAT(i9,1x,(f10.4),2(i3),4(1x,f8.4),8(1x,f10.4),1P,5(1x,e11.4),0P,10(1x,f15.4),
     &       3(i6),1P,58(1x,E11.4),0P,1x,i2,6(1x,f10.4),5(1x,e15.4),3(1x,i1),1x,a30)
 999  FORMAT(i9,1x,f10.4,2i3,8(1x,f10.4),3(1x,f10.4),1(1x,i3),7(1x,f10.4),3(1x,i1),1x,a30)
